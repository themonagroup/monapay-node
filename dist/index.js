...
function verifyWebhook({ rawBody, headers, secret, toleranceSec = 300 }) {
  ...
  if (timestampHeader == null || timestampHeader === '') return { ok: false, reason: 'missing_timestamp' };
  ...
  if (!Number.isFinite(toleranceSec) || toleranceSec < 0) throw new RangeError('toleranceSec phải là số không âm');
  if (Math.abs(Math.floor(Date.now() / 1000) - timestamp) > toleranceSec) {
    return { ok: false, reason: 'timestamp_out_of_tolerance' };
  }
  if (signatureHeader == null || signatureHeader === '') return { ok: false, reason: 'missing_signature' };
  ...
  if (!(timingSafeEqual(expected, supplied) && Boolean(match))) return { ok: false, reason: 'invalid_signature' };
  try {
...
class MonaPay {
  constructor({ baseUrl = DEFAULT_BASE_URL, clientId, clientSecret, username, password, fetch: fetchImpl } = {}) {
    ...
    if (typeof this._fetch !== 'function') throw new TypeError('Môi trường cần fetch built-in (Node.js >=18)');
    ...
    this.webhookLogs = Object.freeze({
      list: (options = {}) => this._request('GET', '/api/v1/webhook-logs', { query: webhookLogQuery(options) }),
      stats: (options = {}) => this._request('GET', '/api/v1/webhook-logs/stats', { query: webhookLogQuery(options) }),
    });
    this.emailConfigs = Object.freeze({
      list: () => this._request('GET', '/api/v1/email-configs'),
      get: (id) => this._request('GET', `/api/v1/email-configs/${segment(id)}`),
      create: (body) => this._request('POST', '/api/v1/email-configs', { body }),
      update: (id, body) => this._request('PUT', `/api/v1/email-configs/${segment(id)}`, { body }),
      remove: (id) => this._request('DELETE', `/api/v1/email-configs/${segment(id)}`),
      verify: (id, body) => this._request('POST', `/api/v1/email-configs/${segment(id)}/verify`, { body }),
      resendVerification: (id, body) => this._request('POST', `/api/v1/email-configs/${segment(id)}/resend-verification`, { body }),
      test: (id) => this._request('POST', `/api/v1/email-configs/${segment(id)}/test`, { body: {} }),
    });
    this.emailLogs = Object.freeze({
      list: (options = {}) => this._request('GET', '/api/v1/email-logs', { query: emailLogQuery(options) }),
      stats: (options = {}) => this._request('GET', '/api/v1/email-logs/stats', { query: emailStatsQuery(options) }),
    });
    this.emailSuppressions = Object.freeze({
      list: () => this._request('GET', '/api/v1/email-suppressions'),
      remove: (email) => this._request('DELETE', `/api/v1/email-suppressions/${segment(email)}`),
    });
  }
  ...
  static fromEnv(env = process.env) {
    ...
    if (env.MONAPAY_CLIENT_ID && env.MONAPAY_CLIENT_SECRET) {
      return new MonaPay({ baseUrl, clientId: env.MONAPAY_CLIENT_ID, clientSecret: env.MONAPAY_CLIENT_SECRET });
    }
...
function webhookLogQuery(options) {
  return {
...
function emailLogQuery(options) {
  return {
...
export { MonaPay, MonaPayError, verifyWebhook, expressWebhook };
